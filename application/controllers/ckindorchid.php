...
class Ckindorchid extends CI_Controller {      
    var $data;
    ...
    function list_data()
    {
        ...
        if (!isset($param['t']))
            $param['t']=0;
        $this->load->model("M_kindorchid");      
        $this->data['r_rows']=0;
        $this->data['r_data']=$this->M_kindorchid->list_data($param['t']); 
    ...
    function save_data()
    {
        ...
        $this->load->model("M_kindorchid");      
        $this->data['r_rows']=0;
        $this->data['r_data']=$this->M_kindorchid->save_data($param['ock_code'],$param['ock_name']); 
        echo 1;
    ...
    function get_data()
    {
        ...
        if(is_array($param))
            sql_quot_all($param);
        if (!isset($param['ock_code']))
            $param['ock_code']='';
        $this->load->model("M_kindorchid");      
        $this->data['r_rows']=0;
        $this->data['r_data']=$this->M_kindorchid->get_data($param['ock_code']); 
        $this->data['r_rows']=count($this->data['r_data']);
        echo json_encode($this->data);
    }
